...
public interface UserTeamRepository extends JpaRepository<UserTeam, UUID> {
    ...
    @Query("SELECT new com.course_work.Sports_Menagement_Platform.dto.UserTeamDTO(ut.id, ut.isCap, ut.isPlaying, u.name, u.surname, u.tel, ut.invitationStatus, u.id) " +
            "FROM UserTeam ut " +
            "JOIN ut.user u " +
            "WHERE ut.team.id = :teamId")
